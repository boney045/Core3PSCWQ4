...
A_N = 1 #intial transfer coefficient A
...
a = 1.5 *ang #the step potentials locations kinda thing
infty = 100*eV #approximating infitite potential energy as a large number(energy)
q = 1.5402 / ang #2/ang #0.0 * ang
A = sqrt( (9*pi^2 - 16*a^2*q^2) / (32*a^2 * q^2) )  #1.0 
...
V0 = abs(Vcalc) #5.0* eV 6.66*eV  #the height of the potential 
...
potentialEnergy[2] = V0
potentialEnergy[3] = 0
potentialEnergy[4] = V0
...
savefig(potentialWellPlot2, "potentialWellPlot2$(replace(replace(string(Dates.now()), ":" => "_"), "."=> "_")).png")



#=range of energy values to calculate t11E over =#
energyStart = 0.01 * eV
energyEnd =  V0 + 20*eV#3.9800035035292347 * eV
energyStep = 0.001 * eV 
...
#=plotting the wave function in the question=#
    ...
    savefig(wavefunctionPlot, "wavefunctionPlot$(replace(replace(string(Dates.now()), ":" => "_"), "."=> "_")).png")
...
savefig(wavefunctionSquaredPlot, "wavefunctionSquaredPlot$(replace(replace(string(Dates.now()), ":" => "_"), "."=> "_")).png")
